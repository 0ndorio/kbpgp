...
(function() {
  var ASP, BaseEccKey, BaseKey, BaseKeyPair, BigInteger, C, ECDH, K, Pair, Point, Priv, Pub, bn, bufeq_secure, generate, iced, konst, make_esc, nbits, nbv, uint_to_buffer, __iced_k, __iced_k_noop, _ref, _ref1, _ref2,
  ...
  Point = require('keybase-ecurve').Point;
  ...
  Pub = (function(_super) {
    ...
    Pub.verification = C.verification_algorithms["default"];

    Pub.prototype.verification = Pub.verification;
    ...
    Pub.prototype.verify = function(_arg, h, cb) {
      ...
      switch (this.verification) {
        case V.butun:
          return this.verify_butun([r, s], h, cb);
        default:
          return this.verify_default([r, s], h, cb);
      }
    };

    Pub.prototype.verify_default = function(_arg, h, cb) {
  ...
  ({
    verify_butun: function(_arg, h, cb) {
      var R, err, hi, r, s, u1, u2;
      r = _arg[0], s = _arg[1];
      err = null;
      hi = this.trunc_hash(h);
      R = Point.decodeFrom(this.curve, R.toBuffer());
      if ((s.signum() <= 0) || (s.compareTo(this.curve.p) > 0)) {
        err = new Error("bad s");
      } else if (!this.curve.isOnCurve(R)) {
        err = new Error("bad R");
      } else {
        u1 = this.curve.G.multiply(s);
        r = R.affineX.mod(this.curve.n);
        u2 = this.R.multiplyTwo(r, R, hi);
        if (!u1.equals(u2)) {
          err = new Error("verification failed");
        }
      }
      ...
      (function(_this) {
        return (function(__iced_k) {
          __iced_deferrals = new iced.Deferrals(__iced_k, {
            ...
            filename: "/home/rnd/dev/kbpgp/src/ecc/ecdsa.iced",
            funcname: "Priv.sign"
          });
  ...
  Pair = (function(_super) {
    ...
    Pair.prototype.verify_unpad_and_check_hash = function(_arg, cb) {
      var data, hash, hasher, sig, verification_algorithms;
      sig = _arg.sig, data = _arg.data, hasher = _arg.hasher, hash = _arg.hash, verification_algorithms = _arg.verification_algorithms;
